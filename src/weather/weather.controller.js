...
async function list(req, res) {
  ...
  const weatherStati = await WeatherStatus.find();
  res.send(weatherStati);
}
...
async function update(req, res) {
  ...
  const { data: { date, city, state, country, temperature, condition } = {} } = req.body;
  weatherStatus.date = date, weatherStatus.city = city;
  weatherStatus.state = state, weatherStatus.country = country;
